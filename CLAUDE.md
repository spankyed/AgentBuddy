...
| `@abuddy/host` | the app runtime, and the app's own features: `features/` is the pack `host` (`application` and `packs`, each `{be,fe}` as any pack's are), and beside it what every pack runs on — the five app services (`/services`), app state (`/app-state`), `/packs` and `/packs/runtime`, `/bus`, `/migrations`, `/secrets`, the frontend's plumbing (`/fe`) and its database opened outside it (`/database`) |
