...
The freshness rule itself lives in `@abuddy/host/build/packages-built`, so `@abuddy/testing` can reach it by package specifier: a relative import of a repo-root script would put the repo root into that bundle's declaration emit and move every declaration it publishes. `scripts/ensure-packages-built.ts` is the command over it and re-exports it, which is how the build scripts inside `@abuddy/ears`, `@abuddy/sdk` and `@abuddy/ui` reach it without naming a package above their own layer — they load host transitively, which `check:specifiers` does not see, and that is build tooling rather than shipped code.
