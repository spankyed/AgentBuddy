...
- `@abuddy/sdk/fe` — pack-facing: `Plugin`, `PackFERegistration`, `safeEvents`, `usePlugin`/`PluginScope` (`fe/actor-system.ts`), `useShell` (`fe/shell.ts`: the app shell's state and commands, typed by `HostShell`; no pack code holds the shell's actor), `openPlugin` (`fe/navigation.ts`), `secretsClient`, the settings composables (`fe/settings.ts`: `useSettingsSection`, `useFeatureSettings`, `useSettingsSave` follow the settings until the calling scope is disposed, so they run in a component's setup and never inside a `computed`; `updateSettings` changes one from outside a scope, such as a machine's action). A change names a section by its own name or a feature by its ref, and the whole path is inside it, etc.
