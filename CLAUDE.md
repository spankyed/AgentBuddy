...
- Anything started without them throws instead of falling back to production. Manual API boots must pass both, pointing at a copy of user data, and a token (`ABUDDY_API_TOKEN`, any random string: the API refuses to start without one, and clients must present it): `cd packages/api && ABUDDY_ENV=development ABUDDY_USER_DATA_DIR=<copy> ABUDDY_API_TOKEN=$(openssl rand -hex 32) NODE_ENV=development API_PORT=3099 BUILT_IN_PACKS_DIR=$PWD/.. node ../../scripts/with-source.mjs node dist/server.js`
- The API takes calls only with that token. Electron main creates one per app run and passes it to the API; the app's windows read it through the preload (`api:token`) and send it when connecting, and in development the API writes it to `apiTokenFile` for local tools (`abuddy dev`, default-setup's watcher), which send it in `API_TOKEN_HEADER` (`@abuddy/sdk/env`). The API listens on `127.0.0.1` only.
