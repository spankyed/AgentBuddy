...
- **Addressing is an envelope**: a send is a message, `{ to, event, from? }`, and `event` arrives exactly as the sender wrote it, so an event may carry any field, `pluginId` or `systemId` included. `from` is the id of the pack that sent it, stamped by the sends `#generated/events` builds — the only place a sender is in scope — and absent on the host's own sends and on an action's through `services.emitter`. Nothing routes or refuses on it: the four diagnostics that report an undeliverable message name it — the bus's two drops, `receiveClientEvent`'s errors, the shell's toast for an in-window send that reaches no plugin, and its warning for a backend send that does. The API's `bus.send` schema names `from` so a client's send keeps it; every other field a client invents is still dropped there. The bus, the API's `bus.send` and its subscription, and the renderer route on `to`; messages sent in go to systems, messages sent out to plugins.
