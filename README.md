...
> **Why they matter**  
> Systems and plugins give Agent-Buddy its extensible shell, but **flows**
> define the _actual behaviour_ of every agent. Because flows are data-driven
> graphs, users can **soft-configure behaviour without redeploying or
...
2. **LLM step** &nbsp;`Process User Message` → summarizes intent.  
3. **LLM step** &nbsp;`Format Response` → converts summary into prose.  
4. **Action step** &nbsp;`Stream to FE` → runs custom code, pushing
