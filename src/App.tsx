...
function App() {
  ...
  return (
    <div className={`h-screen flex flex-col ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <div className="bg-white border-b border-neutral-200 p-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-3">
          {mockActions.slice(0, 6).map(action => (
            <button 
              key={action.id}
              className="py-1.5 px-3 text-sm font-medium bg-white border border-neutral-300 rounded-full hover:bg-neutral-50 transition-colors"
            >
              {action.description}
            </button>
          ))}
