...
| `fe.ts` | Frontend helpers taking the names pack code writes: `actorOf(name)` (a plugin's actor), `navigateToPlugin(name, event?)`, and the `PluginName` type, which lists exactly those names (the pack's own plugins by feature id, its dependencies' by `<packId>/<featureId>`). Only packs with plugins get it |
