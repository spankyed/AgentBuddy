...
| `events.ts` | `SendablePluginEvents` (plugin ID -> the events it receives from this pack's systems, its dependencies' and the host's plugins), `PackSystemEvents` (this pack's systems by feature ID -> the events each receives; dependents name them `<packId>/<feature>`), `SendableSystemEvents` (own systems by feature ID, plus each dependency's as `<dependency>/<feature>`) and `QualifiedSystemEvents` (every system as `<pack>/<feature>`, for `services.emitter`), and the typed `sendToPlugin`/`sendToSystem` sends, which resolve each name to the ref it stands for (a pack without systems sends to its dependencies') |
