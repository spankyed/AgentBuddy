...
- **Reload** (development, `POST /dev/reload` on the API from `abuddy dev` or a built-in pack's watch build, handled by `reloadExternalPack`/`reloadBuiltInPack` in `@abuddy/host/packs/runtime`): loads and registers the rebuilt runtime before shutting the running one down. If the fresh runtime fails to load or register, the running pack is re-registered and stays as it was. Otherwise the old shutdown hooks run, the new `onShutdown` registers, `onInit` runs, external packs re-seed, and the bus `RELOAD_PACK` stops the old and new system ids and starts those still registered, then sends them `CLIENT_CONNECTED`; `PACK_CHANGED` follows for every running system. Teardown (disable, uninstall) sends it too, after stopping the pack's systems.
