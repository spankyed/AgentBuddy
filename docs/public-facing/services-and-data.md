...
| `services.emitter` | `sendToPlugin(plugin, event)` (to the frontend), `sendToSystem(system, event)` (onto the bus) and `sendToBrainSystem({ eventType, payload?, targetFlowId? })` (fires a flow event). `Services` types the first two with your pack's events. Actions run outside any pack, so both name every feature `<packId>/<featureId>`, your own too (`'my-pack/bookmarks'`, `'default-setup/threads'`); a host plugin is named bare (`'application'`). The host sends to the address the name stands for, and throws for a name nothing is registered under, suggesting the `<packId>/<featureId>` form when a bare name matches one pack's feature. |
