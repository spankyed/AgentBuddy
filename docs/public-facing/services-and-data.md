...
| `services.emitter` | `sendToPlugin(plugin, event)` (to the frontend), `sendToSystem(system, event)` (onto the bus) and `sendToBrainSystem({ eventType, payload?, targetFlowId? })` (fires a flow event). `Services` types the first two with your pack's events. Actions run outside any pack, so both name every feature `<packId>/<featureId>`, your own too (`'my-pack/bookmarks'`, `'default-setup/threads'`); the app's own are the pack `host`'s (`'host/application'`). The host sends to the plugin or system the name is, and throws for a name nothing is registered under, suggesting the `<packId>/<featureId>` form when a bare name matches one pack's feature. |
