...
| `system` | `{ entry, events? }` | no | Backend system module. `entry` must **default-export** its `SystemEntry`, declared with `satisfies SystemEntry` (a type annotation loses the system's events). What its plugin receives is its outgoing events; a plugin that also takes events from another feature or another pack declares those itself, with `pluginAccepts()` beside it in `fe/plugin.ts`. `events.incoming` lists event types the bus routes to the system besides those its machine declares. |
