...
| `system` | `{ entry, outgoingEventsType?, sendsTo?, events? }` | no | Backend system module. `entry` must **default-export** its `SystemEntry`, declared with `satisfies SystemEntry` (a type annotation loses the system's events). `sendsTo` lists plugins it sends events to besides its own feature's (other features of the pack **that have a plugin**, dependency plugins, or `application`); each one's `emit` type then accepts this system's outgoing events. Naming one of the pack's own features that has no plugin is a manifest error: nothing could receive the events. `events.incoming` lists event types the bus routes to the system besides those its machine declares. |
