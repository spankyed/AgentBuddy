...
| `system` | `{ entry, sendsTo?, events? }` | no | Backend system module. `entry` must **default-export** its `SystemEntry`, declared with `satisfies SystemEntry` (a type annotation loses the system's events). `sendsTo` lists plugins it sends events to besides its own feature's, named as code names them (other features of the pack **that have a plugin** by id, a dependency's plugins as `<packId>/<featureId>`, or `host/application`), and a plugin the pack can't `sendToPlugin` to at all unless one of its systems names it. Another feature of this pack gains this system's outgoing events in its `sendToPlugin` type; a dependency's plugin or a host plugin keeps the events its owner declares it receives, since only the pack that owns a plugin can handle a new event. Naming one of the pack's own features that has no plugin is a manifest error, and naming a dependency's plugin that dependency declares no events for is a build error: nothing could receive the events. `events.incoming` lists event types the bus routes to the system besides those its machine declares. |
| `plugin` | `{ entry, default? }` | no | Frontend plugin module. `entry` must **default-export** its `Plugin`, which carries the plugin's `id`, `label`, `icon` and `isPinned`. `default: true` opens this plugin when the app starts — at most one of your features may claim it, and across the app the first pack to register one wins. Without a claim, your pack's first plugin feature is its default |
