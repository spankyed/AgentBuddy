...
TypeScript 5.9.3, vue-tsc 3.2.6, Vite 7.0.6, esbuild 0.25.12. Branch `spike/ts-specifiers` (deleted after recording this). The codemod rewrote 312 specifiers in sdk (81 files), 38 in ui (17 files incl. `.vue` scripts) and 38 in host src and tests (16 files).

| Check | Result | Evidence |
|---|---|---|
| 1. Emit: JS | pass | `tsc` rewrote every relative specifier in `packages/abuddy-sdk/dist/**/*.js` to `.js` (a grep for relative `.ts` specifiers in the JS found none; all 204 left were in `.d.ts`); `import('…/dist/ears/index.js')` loads under plain Node. The UI's `.ts` modules likewise, with the flag passed to the JS emit step. |
| 1. Emit: declarations | pass, with a caveat | `rewriteRelativeImportExtensions` doesn't touch declaration emit: `dist/index.d.ts` keeps `from './framework/index.ts'`, and vue-tsc's `.d.vue.ts` keeps `.ts` too. TypeScript still resolves them for consumers: `published-sdk-types` and `published-exports` passed under node16 and bundler with `noImplicitAny`, and attw and `publint --strict` were clean, all without consumer flags. |
| 2. Published | **fail** | attw (sdk, ui), publint, `published-sdk-types`, `published-ui-types`, `published-exports`, `fe-bundler-host-registry` passed. `test:packaged-authoring` failed at `abuddy build`: `FE bundle failed: Could not resolve "./extensions.ts" from "node_modules/@abuddy/ui/dist/components/tiptap/TiptapEditor.vue?vue&type=script&setup=true&lang.ts"`. |
| 3. SFCs | **fail** | In the monorepo it works: `typecheck:ui`, the renderer's `vue-tsc --build`, its Vite build and Vitest passed. But `@abuddy/ui` ships `.vue` files as source for the pack's Vite build, and their scripts now import `./extensions.ts`, `./monaco-config.ts`, `../composables/useContextMenu.ts`, … while the package contains only the compiled `.js` of those modules. Also, with `rewriteRelativeImportExtensions` vue-tsc reports TS2876 ("unsafe to rewrite … resolves to ./X.vue") on every `.vue` import, so the UI can only set the flag on its tsc JS emit step. |
| 4. Consumers | pass (fixable) | TS5097 in any program compiling the source: api (191 errors; its tsconfig emits, so it needs `allowImportingTsExtensions` + `rewriteRelativeImportExtensions`), host (161), ui (59), cli (129) and testing (129) need `allowImportingTsExtensions`; `bundle-package.ts`'s declaration tsc for `@abuddy/testing` needs `--allowImportingTsExtensions`. renderer, default-setup and main already allow it (their base configs) and passed unchanged. |
| 5. End to end | not run | Stopped at the packaged-authoring failure above. |

**Why it fails:** the shipped SFC sources would need their `.ts` specifiers to point at emitted `.js` files, and the only ways to get there break a decision:
- rewriting specifiers in the shipped `.vue` files at build time, or resolving `.ts` → `.js` in the pack's bundler, is exactly the post-processing Decision 1 rules out (and the bundler variant would break authors using their own Vite);
- shipping the `.ts` sources next to the `.js` would load two instances of the same module (the SFC's `.ts` copy and the package export's `.js`), splitting module state;
- keeping `.js` in `.vue` scripts only is two conventions, which Decision 2 rules out;
- compiling SFCs to JS in the package means a Vue bundler, not `tsc`.

**Decision:** keep `.js` specifiers in sdk, ui and host. Nothing else changed. Revisit if `@abuddy/ui` stops shipping `.vue` source (precompiled SFCs) or TypeScript/vue-tsc gain a supported way to rewrite specifiers in SFCs.

## Spike result: failed, `.js` stays

Run on a throwaway branch (`spike/ts-specifiers`, deleted afterwards) with TypeScript 5.9.3, vue-tsc 3.2.6, Vite 7.0.6 and esbuild 0.25.12. The codemod rewrote 312 relative specifiers in 81 SDK files, 38 in 17 UI files (including `.vue` `<script>` blocks) and 38 in 16 host files. The one import of a hand-written declaration (`fe/electron-api.d.ts` → `./speech-event.js`) has no `.ts` source and kept `.js`.

| Check | Result | Evidence |
|---|---|---|
| 1. Emit | ❌ | **SDK:** tsc-emitted JS has only `.js` relative specifiers. The emitted `.d.ts` files keep `.ts` (`dist/index.d.ts`: `from './framework/index.ts'`), because `rewriteRelativeImportExtensions` doesn't rewrite declarations. **UI:** the tsc-emitted JS is rewritten, but `.d.ts` (3 files) and `.d.vue.ts` (5 files) keep `.ts`. The **11 `.vue` files shipped as source** keep `.ts` specifiers to modules that exist in `dist/` only as `.js` (`dist/design/ImageLightbox.vue`: `from '../composables/useContextMenu.ts'`, next to `useContextMenu.js`). |
| 2. Published | ✅ SDK / ❌ UI | **SDK:** `attw` exit 0, `publint --strict` "All good!", CLI `published-sdk-types` 5/5 and `published-exports` 2/2. Declarations with `.ts` specifiers resolve under node16 and bundler with no consumer flags. **UI:** fails through check 5. |
| 3. SFCs | ❌ | `typecheck:ui` fails with `rewriteRelativeImportExtensions` enabled: 16× TS2876 ("This relative import path is unsafe to rewrite because it looks like a file name, but actually resolves to "./SimpleMonacoEditor.vue""), because vue-tsc resolves `X.vue` to a virtual TS file. It passes with `allowImportingTsExtensions` alone and the rewrite flag passed only to the tsc JS-emit run. The renderer's `vue-tsc --build` passes. A pack's Vite build of the shipped SFCs fails (check 5). |
...
| 5. End to end | ❌ | `npm run test:packaged-authoring` exit 1: `FE bundle failed: Could not resolve "./editor-config.ts" from "node_modules/@abuddy/ui/dist/components/tiptap/TiptapEditor.vue?vue&type=script&setup=true&lang.ts"`. The remaining suites weren't run on the spike, since this settles it. |
...
- a script rewrites them while copying, which Decision 1 rules out;
...
**Decision:** per Decision 2, all three packages keep `.js` specifiers. Nothing else changed.
