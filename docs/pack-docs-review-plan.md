...
## 5. Examine the example pack

Look at `example-pack/` (or whatever scaffold `abuddy init` produces). Assess whether it demonstrates the full surface area or only a subset. Note gaps between what it shows and what's possible.

## 6. Trace the build and packaging pipeline
