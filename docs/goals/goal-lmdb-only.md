...
- **Multiple processes.** The `db:*` scripts (`packages/api/scripts/db`) open the store read-write while the app runs. With LMDB-only, each process sees the other's writes. See docs/goals/goal-abuddy-db-cli.md.
...
- The `db:*` scripts open the env with the app's flags, coordinated with docs/goals/goal-abuddy-db-cli.md.
