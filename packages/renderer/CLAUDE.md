...
- **`backendListener`:**
  ...
  - Each event's `pluginId` routes it: `host/application` and `_meta` go to this actor (`CLIENT_CONNECTED`, `APPLICATION_HOTKEYS`, `PLUGIN_VISIBILITY_UPDATED`, `APPLICATION_RESTORE_LAST_PLUGIN`, `SYSTEM_ERROR`); anything else goes to `system.get(pluginId)`.
