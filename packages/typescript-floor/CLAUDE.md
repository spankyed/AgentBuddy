...
A private workspace that installs nothing but `typescript@5.7.3`, the oldest TypeScript `@abuddy/ears`, `@abuddy/sdk` and `@abuddy/ui` support. The SDK's and UI's `typescript` peer is `>=5.7`, the version `ai` 7's declarations need. It has no source.
...
The pinned version, the `typescript` peer ranges in `packages/abuddy-sdk/package.json` and `packages/abuddy-ui/package.json` (`@abuddy/ears` has no TypeScript peer: it ships declarations only), and the `'5.7'` key in `TSC_VERSIONS` move together. Update the root `CLAUDE.md` and `docs/public-facing` wherever they name the version. Pin an exact version, not a range, so the specs test the actual floor.
