...
| `./migrations` | `migrations/index.ts`, `migrations/app/*.ts` | The migrations runners, `runAppMigrations(registry)` and `runPackMigrations()`, and the host's own app migrations ([its CLAUDE.md](src/migrations/CLAUDE.md)) |
...
| `./services` | `services/{index,app-data,trace-store,inference,secrets}.ts` | `createHostRuntime(...)`, `createHostServices(...)`. Only the four app-implemented services and the index (`tests/boundaries.spec.ts`) |
...
- Packs never read it, and their settings don't hold it: resetting settings leaves it alone (`default-setup/tests/unit/settings-reset-app-state.spec.ts`). `appData.reset()` clears it with the rest of the database; the migrations that end the reset record the version again.
...
- `createHostRuntime({ store, engine, transport, appVersion, packs })` (`index.ts`) is the only place the app's `HostRuntime` (`@abuddy/sdk/runtime`) is assembled: the given bus and version, the engine's query face (`engine.query`, which `bindHost` installs), the app's registry as `packs` (the SDK's lookups read it) and `createHostServices({ store, engine: engine.admin, packs })`, the four host-implemented services (`appData`, `traceStore`, `inference`, `secrets`) over the app's LMDB store (`LmdbStore` from `@abuddy/ears/lmdb`) and the engine's admin face. The API binds it in `openAppStore()` (`packages/api/src/setup/backend.ts`), once it has opened the store.
...
- `app-data.ts` (`createAppData(store, engine, registry)`) handles reset, the whole app's: it clears the engine's memory (`engine.clear()`, the admin face), `store.reset()`, then `secretsStore.clearAll()` once the store is open again, then runs each registered pack's `boot.onInit`, the boot seeds (`registry.runRegisteredBootSeeds(orchestrateDeclarativeSeed)`) and the host's `runAppMigrations(registry)` (`../migrations`). default-setup's reset actions only call it. `hasOnboarded()` and `completeOnboarding()` read and write `AppState`. It also handles backup export and import, and backup info. After an import, or a failed import, it clears memory and hydrates the store again.
...
- `migrations/`: `runner` (both runners on the SDK test host, each migration once, versions recorded in `AppState`) and `app-state-0.3.15` (the host's 0.3.15 migration on old-shaped settings, run twice, and the runner on data from before `AppState` and on new data).
- `packs/`: `bundle`, `dependencies` (`checkDependencies`, publish staging), `dev-server`, `discovery`, `event-validation-map`, `host-info`, `module-bridge`, `partition-policy`, `registration`, `pack-protocol` (the `pack://` MIME table), `registered-lookups` (each SDK lookup through the functions packs call: registering, unregistering, collisions with rollback), `two-registries`, `registry-state` (no module-level state in the registry and lookup modules), `backend-contributions` (the command and settings stores), `shutdown-hooks`, `staging`, and `updater` (`findLatestRelease`, with `fetch` mocked).
...
- `services/`: `inference` (provider URLs, model kinds) and `host-runtime` (`createHostRuntime`'s members, and a reset's order: stores, `onInit`, migrations).
- `build/`: `source-resolution`, `shared-deps` (`APP_ONLY_EXPORTS` stay out of both bridges).
- Related suites elsewhere:
  - `packages/api/tests/unit/` (`secrets`, `bus-client-connected`, `bound-runtime`, `log-capture`, `host-data-services`, `restart-persistence`, and `app-reset`: a reset on the built-in packs)
...
- Adding a host service means updating `HostServices` and `HostRuntimeServices` in the SDK, `HOST_SERVICE_NAMES` (`pack-registration.ts`), `createHostServices` (`services/index.ts`, with the implementation in `services/<kebab-case key>.ts`) and `HOST_SERVICE_KEYS` in `tests/boundaries.spec.ts`. All are type-checked against the SDK types. Helpers that aren't a service don't go in `services/`.
