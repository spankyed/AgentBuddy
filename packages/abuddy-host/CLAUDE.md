...
| `./database` | `database/{index,open,schema,layout,version,running}.ts` | A data dir's database opened outside the app (`abuddy db`), through the composition the API's boot uses (see Database) |
...
- `openAppDatabase({ env, userDataDir, readOnly?, supportedVersion?, log? })`: for a data dir outside the app. It reads the installed packs' schema (`readInstalledSchema`, `schema.ts`: the built-in packs' `host-packs/<id>/types/snapshot.json` manifests and the external packs' `abuddy.json`, minus those `pack-registry.json` disables; entity types and relation kinds from all, the partition policy from the built-in packs through `appPartitionPolicy`, as the registry does; no pack code runs; a data dir with no published built-in packs throws), finds the layout (`findAppDataPaths`, `layout.ts`: the stores at the data dir's root (packaged) or under `.data/` (source), from `appDataPaths` in `@abuddy/sdk/utils`; none or both throws), opens and hydrates the primary partition, installs the engine's query face, and reads the data's version (`AppState.version`, else `host.json`'s). With `supportedVersion`, other data than that major.minor throws `DataVersionMismatchError` (`checkDataVersion`, `version.ts`; a prerelease counts as its release, data with no version passes). `close()` uninstalls the engine, closes the store and throws when a write failed (the store's `close()` returns its failed writes, the final flush's included). `readOnly` opens the files read-only (LMDB and the sink: a write throws).
- `findRunningApp({ userDataDir, apiPortFile })` (`running.ts`): why an app runs on the data dir, or `null`: Chromium's `SingletonLock` (`<host>-<pid>`) held by a live process (a lock from another host, or unreadable, counts), or the API's port file naming a port that answers on `API_HOST`. A stale file doesn't count.
