...
| `./migrations` | `migrations/index.ts`, `migrations/legacy-app-state.ts` | The migrations runners, `runAppMigrations(registry)` and `runPackMigrations(registry, packs)`, which first move the app's state from before `AppState` ([its CLAUDE.md](src/migrations/CLAUDE.md)) |
...
- Before 0.3.15 it lived in default-setup's Settings row (`data.internal`); the migrations runners move it first, whatever the app version (`migrations/legacy-app-state.ts`, `tests/migrations/legacy-app-state.spec.ts`).
...
- `migrations/`: `runner` (both runners on the SDK test host, each migration once, versions recorded in `AppState`) and `legacy-app-state` (moving the app's state out of old-shaped settings, run twice; the runners on data from before `AppState` at, below and before the release version, on new data, and after a failed move).
...
- `services/`: `inference` (provider URLs, model kinds) and `host-runtime` (`createHostRuntime`'s members, and a reset's order: shutdown hooks, stores, `onInit`, migrations, seeds, external packs included).
