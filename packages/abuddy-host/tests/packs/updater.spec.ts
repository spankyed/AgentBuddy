...
describe('findLatestRelease', () => {
  ...
  it('fails with the cause when GitHub refuses: rate limit, private or missing repository', async () => {
    ...
    await expect(findLatestRelease('acme/pack')).rejects.toThrow(/rate limit is used up[\s\S]*set GITHUB_TOKEN/);
    vi.stubGlobal('fetch', vi.fn(async () => new Response('Not Found', { status: 404 })));
    await expect(findLatestRelease('acme/pack')).rejects.toThrow(/not found[\s\S]*set GITHUB_TOKEN/);
    ...
    await expect(findLatestRelease('acme/pack')).rejects.toThrow(/rate limiting these requests/);
    vi.stubGlobal('fetch', vi.fn(async () => new Response('nope', { status: 401 })));
    await expect(findLatestRelease('acme/pack')).rejects.toThrow(/needs credentials[\s\S]*set GITHUB_TOKEN/);
  });
