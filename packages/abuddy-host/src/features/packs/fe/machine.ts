...
const packsState = setup({
  ...
  actions: {
    ...
    sendInstall: ({ event }) => {
      ...
      untypedSendToSystem(HOST.packs, { type: 'INSTALL_PACK', packSlug: ev.packSlug, source: ev.source });
    },
    ...
    sendUninstall: ({ event }) => {
      ...
      untypedSendToSystem(HOST.packs, { type: 'UNINSTALL_PACK', packId: ev.packId });
    },
    ...
    sendToggleEnabled: ({ event }) => {
      ...
      untypedSendToSystem(HOST.packs, { type: 'TOGGLE_PACK_ENABLED', packId: ev.packId });
    },
    ...
    sendUpdate: ({ event }) => {
      ...
      untypedSendToSystem(HOST.packs, { type: 'UPDATE_PACK', packId: ev.packId });
    },

    sendCheckUpdates: () => {
      untypedSendToSystem(HOST.packs, { type: 'CHECK_FOR_UPDATES' });
    },

    sendRefresh: () => {
      untypedSendToSystem(HOST.packs, { type: 'GET_INSTALLED_PACKS' });
    },
  },
}).createMachine({
  id: HOST.packs,
