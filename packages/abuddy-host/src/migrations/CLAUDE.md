...
`app/` holds the host's own migrations, which move data no pack's own migration can: the app's state, and installed external packs' stored settings; `app/index.ts` lists them, over the app's registry. `app/0.3.15.ts` moves the `internal` section of the settings (default-setup's Settings row, read untyped: it's data in the shape before 0.3.15) into `AppState`: onboarding, the version, pack versions and seed hashes, with the single `seedHash` of older versions filed under each built-in pack with a boot seed. It keeps what `AppState` already records and writes only what differs. It also moves the app shell's state out of the settings (`plugins._meta`: which tabs show, the plugin last open) into `AppState`, each id onto its plugin's ref among every registered plugin (a bare id a built-in pack has being the built-in plugin's, which ran under it) and one naming none dropped; and the host's and installed external packs' stored plugin settings from their features' bare ids onto their refs (`addressPluginKeys` from `@abuddy/sdk/framework`, which default-setup's 0.3.15 and its settings replace run for their own). A pack that isn't loaded when it runs keeps its bare keys. It runs before default-setup's 0.3.15, which drops the section. Delete it with the other migrations once 0.3.15 is below the oldest version upgrades are supported from.
