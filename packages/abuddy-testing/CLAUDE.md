...
- **`startApp({ systems })`** (`src/app.ts`) — the named registered systems under `@abuddy/host/bus`'s `createBusMachine` (the API's bus core), on `testRootEvents`.
  ...
  - `runFlow` and `flowTrace` drive default-setup's brain (designated `brain` and `settings`). `runFlow` grants the root flow role and sends `START_BRAIN`/`RESTART_BRAIN` and `TRIGGER_BRAIN_EVENT`, then waits on the brain's `TNODE_SPAWNED`/`TNODE_UPDATED` events. Steps whose runtime sets `waits` (keep-alive) count as finished-waiting. Trace node rows are captured as reported, since the brain clears them when it stops. `flowTrace` finds a subflow by its flow's label (the subflow step's `flowRef`). `runFlow`'s timeout covers its sends and settling too.
