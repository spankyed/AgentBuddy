...
- **`setupPackTests({ seedRuntime, registration?, packDir? })`** — from a vitest setup file. `packDir` defaults to the nearest `abuddy.json` at or above the injected vitest project root (else the working directory).
  ...
  - Before each test it empties the database and media store, and fails a concurrent test: the database, mocks and apps are per file.
...
- **`startApp({ systems })`** (`src/app.ts`) — the named registered systems under `@abuddy/host/bus`'s `createBusMachine` (the API's bus core), on `testRootEvents`.
  - Members: `connect`, `send`, `emitted`, `nextEmit`, `settle`, `system`, `stop`.
  - Until `connect`, the bus drops events for systems (client events, `sendToSystem`, `fire` steps, schedule ticks), as the app's bus does before its first client. The app records what it dropped, and `runFlow`/`nextEmit` errors name it.
  - `stop` stops the bus, rejects pending waits with "The test app stopped" (marking in-flight calls handled, so a wait nobody awaits isn't an unhandled rejection), and, once no app runs, calls every registered pack's `boot.onShutdown` (`getBootHooks`), as the API does when it tears a pack down. Stopped actors never reach their own cleanup (default-setup's `killBrain`), so module-level state (cron jobs, ad-hoc brain listeners, the flow actor registry) is cleared there: default-setup's `features/hooks.ts`.
...
- **`mockInference(reply, replies?)`** — mocks `services.inference` for the current test with `fakeInference(reply, replies)` (text, agents, and embedding, image, speech and transcript replies) (`@abuddy/sdk/testing`) and returns the fake, whose `calls` a test asserts.
...
- **Proofs:**
  - `tests/fixtures/external-pack` unit-tests its memo seeds, its memos system, a memo flow on default-setup's brain, and the harness's isolation (mocks, waits ended by stop, dropped events), run by `test:external-pack`.
  - `default-setup/tests/unit/harness-app-stop.spec.ts`: a real schedule in one test doesn't tick into the next.
  - `abuddy-cli/tests/harness/harness-setup.spec.ts`: `vitest run --root <pack>` from elsewhere, concurrent tests rejected, `abuddy add feature` adding the setup to a pack without one.
  - `test:packaged-authoring` also type-checks the packed declarations with `skipLibCheck: false`.
