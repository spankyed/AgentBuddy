...
- Component-specific CSS goes in `<style scoped>` as plain CSS (no `@apply`), as in `dialog.vue` (`.dialog-overlay`, `.dialog-content`). Unscoped `<style>` is kept for styles that must reach rendered or third-party DOM (`TiptapEditor.vue` `@import "./tiptap-theme.css"`, `UnifiedMonacoEditor.vue`, `ColorPicker.vue`, `JsonViewerDialog.vue`). `tiptap/extensions.ts` imports `highlight.js/styles/github-dark.css`.
