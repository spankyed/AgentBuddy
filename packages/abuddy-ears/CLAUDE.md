...
- `PersistenceSink` is the port: the engine calls it on every create, destroy, attribute write and relation write (`onCreateEntity`, `onPutAttr`, `onAddRelation`, …). Bulk loads (`admin.bulkLoadAttr`, `admin.addToIndex`) don't.
