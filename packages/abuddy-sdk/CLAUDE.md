...
- `database-console/`: the Database console's code runners, `runQueryCode` (the read helpers) and `runTransactionCode` (read and write helpers) over the installed engine, with the `EARS` the caller passes, `WRITE_HELPER_NAMES` and `getSchemaStats`. default-setup's Database plugin and query step and `abuddy db query`/`exec`/`repl` run code with them (`tests/database-console/`).
