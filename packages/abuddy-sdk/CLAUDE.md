...
- `env/`: `resolveAppContext()`, the single resolver for environment and data paths, `getAppVersion()` (the bound runtime's `appVersion`), and what local tools need to reach a running app's API: `API_HOST`, `API_TOKEN_HEADER`, and `readApiEndpoint(apiPortFile)`, which returns the `{ port, pid }` a running API published, or `null` for a file a crashed run left behind.
