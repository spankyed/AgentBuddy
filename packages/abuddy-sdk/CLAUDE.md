...
- `env/`: `resolveAppContext()`, the single resolver for environment and data paths, `getAppVersion()` (the bound runtime's `appVersion`), and the API's address and token header for local tools (`API_HOST`, `API_TOKEN_HEADER`).
