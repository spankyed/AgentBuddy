<template>
  ...
  >
    ...
    <template v-if="AddHandle && editable && sourceHandles && sourceHandles.length > 0">
      <component
        :is="AddHandle"
        ...
        @create-connected="(nodeType: string, sourceHandle?: string) => $emit('create-connected', nodeType, sourceHandle)"
        ...
        @remove-handle="(handleId?: string) => $emit('remove-handle', id, handleId)"
      />
    </template>
    <!-- Default single source handle (AddHandle with + icon) - only in edit mode -->
    <component
      :is="AddHandle"
      v-else-if="AddHandle && editable && effectiveShowSourceHandle"
      :node-id="id"
      ...
      @edge-select="(nodeId: string, handleId?: string) => $emit('edge-select', nodeId, handleId)"
    />
    <!-- Plain invisible source handle for edge anchoring (view mode or no AddHandle) -->
    <Handle
      v-else-if="effectiveShowSourceHandle"
      type="source"
