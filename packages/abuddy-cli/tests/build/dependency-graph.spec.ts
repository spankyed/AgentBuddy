  ...
  it("fails a dependent whose sendsTo names one of its plugins", () => {
    const sender = {
      ...
      'src/system.ts': [
        ...
        "export const relaySpec = defineSystem<{ type: 'RELAY' }, OutgoingRelayEvents>();",
        'const entry = { spec: relaySpec, machine: setup({ types: relaySpec.types }).createMachine({ id: "relay" }) } satisfies SystemEntry;',
        'export default entry;',
      ].join('\n'),
