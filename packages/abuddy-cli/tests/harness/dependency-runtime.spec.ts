...
it('connects to default-setup settings', async () => {
  ...
  // One SDK: this pack's feature settings (registered by the harness) reach default-setup's settings,
  // and the repositories default-setup's runtime registered are the test's
  expect((loaded.data as { plugins: Record<string, unknown> }).plugins.widgets).toEqual({ size: 3 });
  expect((repository as unknown as Record<string, unknown>).settingsQueries).toBeDefined();
});`);
