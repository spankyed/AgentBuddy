...
| `db <command>` | `commands/db/` | The app's database, offline, through `@abuddy/host/database` (`openAppDatabase`, `findRunningApp`): `query`/`exec`/`repl` (`code.ts`, `repl.ts`: `@abuddy/sdk/database-console`'s runners, with `EARS` from the installed packs, `consoleScope`), `inspect`, `export`, and `import`/`reset`/`clear-settings` (`destructive.ts`: dry runs without `--force`). `target.ts`: `-d`/`-b`/`--data-dir`, prints the target on stderr, refuses writes while an app runs on it, warns reads; the version check against `supportedAppVersion()` (`src/app-version.ts`: the root `package.json` from source, stamped into the bundle by `scripts/bundle-package.ts`) and `--ignore-version` for reads. `lmdb` is a dependency, external in the bundle. Spec: `tests/cli/db.spec.ts` |
