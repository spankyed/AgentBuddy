...
| `fetch-deps` | `commands/fetch-deps.ts` | Resolution chain: `file:` path; else workspace → the app configured for `abuddy test` → installed apps (`resolveFromMachine`); else the `.abuddy/deps` cache; else GitHub releases (registry lookup is a stub). `resolveDepArtifacts` is what `build` uses |
...
Each problem names the facade exports that reach it (`exportsReaching`). Specs: `tests/build/facade-gate.spec.ts`, `facade-typing.spec.ts`, `types-bundler-determinism.spec.ts`.
