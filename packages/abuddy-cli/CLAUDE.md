...
| `install <source> [-d\|-b]` | `commands/install.ts` | `detectSource` → `installPackFromLocal` / `installPack`, checking `hostVersion` from `readHostVersion(userDataDir)`. Bare names go to `resolveFromRemoteRegistry`, which always throws (not implemented) |
