...
Rows go through those hooks, which call `noteCommands`/`libraryCommands`, whatever pack seeds them. A pack depending on default-setup seeds with its formats by naming them (`{ "path": "src/seeds/notes", "format": "default-setup:notes" }`) and gets the same rows. `tests/unit/seed-parity` compares seeded rows against goldens recorded from the previous pipeline. Its `default-setup.json` follows the pack's own sources, and the SDK's `dist` too, since a compiled action inlines `@abuddy/sdk/actions` — so a bundler or tsc change can churn it. An action also inlines its `_helpers/`, which is what makes the blast radius surprising: editing one helper re-hashes every action that imports it, and two helper edits have moved 47 of the 62 golden rows. Only `sourceHash` and `rowSha256` move in that case, and re-recording is a deliberate step (`npm run seed-parity:update`), because the new hash reseeds every unedited copy of those rows on each user's next boot.
