...
Entity types and relation kinds come from `__generated__/ears.ts`, generated standalone from `abuddy.json` by `abuddy generate-entries`. The pack-entry registers the entity types and relation kinds `abuddy.json` declares. `ears.ts` also names the SDK's own: Relation and the flow model (Flow, Node, TNode, Action, Prompt; contains, transitions_to, instance_of, spawned, tracked). The SDK defines their shapes (import `ActionEntity`, `PromptEntity`, `FlowEntity`, `NodeBase`, `TNodeEntity` from `@abuddy/sdk`) and owns their repositories (`flowRepository`, `tnodeRepository`, `actionRepository`, `promptRepository` from `@abuddy/sdk/repositories`): the flows, actions and prompts repositories here (`flowsQueries`/`flowsCommands`, `actionQueries`/`actionCommands`, `promptQueries`/`promptCommands`) and `brainCommands.updateTNodeResult` expose their methods by reference (`byId: actionRepository.byId`, never a wrapper re-declaring the signature), so this pack's code reaches all its data through `repository`, and add the plugins' views (`connectedData`, `extendedData`, paging, a flow created with its entry node). The root flow is only the flow with the root role: the flows system sends it with its data (`FLOWS_CONNECTED`'s `rootFlow`, again after an import and on `PACK_CHANGED`) and changes it on `SET_ROOT_FLOW`, and the brain sends the root flow it started with (`BRAIN_STARTED`'s `rootFlowId`), which the Flows and Brain settings compare to offer a restart. No setting records either. Exporting actions and prompts to seed files isn't a view: `export-actions.ts` and `export-prompts.ts` beside the repositories read through them. Settings, Document, Collection and Note are default-setup's, declared in `abuddy.json` with their shapes (`SettingsEntity` in `features/settings/be/types.ts`; `DocumentEntity`, `CollectionEntity`, `ContentSection` in `features/library/be/types.ts`; `NoteEntity` in `features/notes/be/types.ts`). The app's own state (onboarding, versions, seed hashes) isn't settings: it's the host's `AppState`, which this pack never reads; onboarding goes through `services.appData.hasOnboarded()`/`completeOnboarding()`. `PackShapes['Node']` is the pack's step node union (`NodeEntity`, each step's `XNode extends NodeBase`). A plugin's settings are stored under its ref (`plugins['default-setup/threads']`), which any installed feature that declares settings or has a plugin may have (`getFeaturesWithSettings`), and nothing else is (`features/settings/plugin-settings.ts`; which tabs show and the plugin last open are the host's, in `AppState`): a name resolves to its key where it's written, with `ref(name)` from `#generated/ref` (`pluginSettings(settings, name)` reads a slice by name from settings in hand), and the store takes only a `FeatureRef` (the repository's `getPluginSettings` and `updatePluginSetting`). A name that arrives as a string is parsed once where it arrives, with `settingsQueries.pluginSettingsRef` (the SDK's `resolveRegistered` over the installed features with settings, `getFeaturesWithSettings` from `@abuddy/sdk/framework`, a disabled pack's included), which throws naming the ref it likely meant: `services.settings` (which dependents and actions call, so actions write `'default-setup/code'`) and the settings system's `UPDATE_SETTINGS` (the settings plugin's `SETTINGS.UPDATE`). A name resolved in the store would be read as this pack's whoever wrote it (`tests/unit/plugin-settings-keys.spec.ts`). Keys stored before 0.3.15 under a bare feature id are moved onto refs by the host's 0.3.15 migration, before this pack's. Every write to the Settings row, whoever makes it (the settings system, another feature's system, an action through `services.settings`, a seeder or migration), goes through the repository's one writer, which checks the whole next document's shape once (`settingsProblems` in `features/settings/document.ts`: an object, known sections, every plugin key a ref) and throws `SettingsRefusedError` otherwise, then tells the settings system (`onSettingsWritten`); the settings system then tells each feature whose settings changed, once (`FEATURE_SETTINGS_UPDATED`). The writes build the next document with `document.ts`'s pure operations (`setIn`, `removeIn`), which take a key such as `__proto__` as data. Settings replaced wholesale (`REPLACE_SETTINGS`, the settings editor), the one edge that carries arbitrary plugin keys, are checked against the installed features with settings too (`refProblem`): a changed slice must be one, and an uninstalled pack's slice stays while unchanged. They are the settings in effect: the store keeps only what they change from the defaults (`changesFrom`), and the settings plugin hears `SETTINGS_SAVED` or `SETTINGS_REFUSED` with the reasons, as it does for every `UPDATE_SETTINGS`; a refusal is the sender's to fix, so only an unexpected error is reported as a system error. The plugin holds the last change's status (`save`), which `useSettingsSaveStatus` and the settings editor show, so a form says "Saved" only for a change the store stored. After an app reset it tells every feature its settings with no changes, since the feature's data was reset with them, and so after a backup import, which runs inside `settingsCommands.whileReplacingData`: the writer tells the listeners a replacement is running and, once it settles, that it ended, so those arrive in order with the writes they bracket and nothing is told a change in between. While one runs the settings system takes no pack change and refuses an app reset, which would race the import over the same stores. The host keeps TNode out of persistence. API keys aren't entities: Settings → Secrets manages them through the host's `secrets` procedures and `services.secrets`, and CLI path overrides live in the code plugin's settings (`plugins['default-setup/code'].cliPaths`), which the code feature's CLI resolution reads (`features/code/be/utils/resolve-cli.ts`: `resolveForService`, `testCli`).
