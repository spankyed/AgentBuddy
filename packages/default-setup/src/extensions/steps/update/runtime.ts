...
import { assertEntityType, createEntityRow, reportError, stepFields } from '../create/runtime';
...
export async function handler(t: TNodeEntity, node: unknown, ctx: ExecutionContext, actor: unknown) {
  ...
  try {
    ...
    if (id && findById(id)) {
      ...
      brainInspect(`Updated ${id} in node: ${n.label}`, { fields });
    ...
    switch (n.onMissing ?? 'fail') {
      case 'ignore':
        brainInspect(`${missing}; update node "${n.label}" ignores it`);
      ...
      case 'create': {
        ...
        brainInspect(`${missing}; update node "${n.label}" created ${row.id}`, { row });
