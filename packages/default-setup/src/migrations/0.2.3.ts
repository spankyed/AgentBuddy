...
export const migration: PackMigration = {
  ...
  up: () => {
    ...
    if (codeSettings && codeSettings.terminalScripts === undefined) {
      repository.settingsCommands.updateSettings('plugin', 'code', ['terminalScripts'], [
    ...
    if (threadsSettings?.chat?.clickToChat === false) {
      repository.settingsCommands.updateSettings('plugin', 'threads', ['chat', 'clickToChat'], true);
    }
    ...
    if (threadsSettings?.chat && threadsSettings.chat.quickPromptNumberKeyInserts === undefined) {
      repository.settingsCommands.updateSettings('plugin', 'threads', ['chat', 'quickPromptNumberKeyInserts'], true);
    }
  }
};
