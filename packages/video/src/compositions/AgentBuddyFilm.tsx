...
import {shots, totalFrames} from '../ui/timeline';
import {
  BoardSurface,
  ChatSurface,
  CodeSurface,
...
function ShotSurface({id, variant}: {id: string; variant: Variant}) {
  const frame = useCurrentFrame();
  if (id === 'notes') return <NotesSurface frame={frame} variant={variant} />;
  if (id === 'chat') return <ChatSurface frame={frame} variant={variant} />;
  if (id === 'board') return <BoardSurface frame={frame} variant={variant} />;
  if (id === 'code') return <CodeSurface frame={frame} variant={variant} />;
  if (id === 'workflow') return <WorkflowSurface frame={frame} variant={variant} />;
  if (id === 'montage') return <MontageSurface frame={frame} variant={variant} />;
  return <FinalSurface frame={frame} variant={variant} />;
}
