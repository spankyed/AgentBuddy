...
function addAttribute(entityID: ECS.EntityId, attributeType: string, value: ECS.AttributeValue): void {
...
function addRelation(sourceEntityID: ECS.EntityId, relationType: string, targetEntityID: ECS.EntityId, info?: ECS.AttributeValue): ECS.EntityId {
...
function updateAttribute(entityID: ECS.EntityId, attributeType: string, newValue: ECS.AttributeValue, index = 0) {
  if (!attributeStore[attributeType]) {
...
function updateAttributeByCriteria(entityID: ECS.EntityId, attributeType: string, criteria: ECS.AttributeValue, newValue: ECS.AttributeValue): void {
...
function updateRole(entityID: ECS.EntityId, oldRoleName: string, newRoleName: string): void {
...
function updateRelation(relationEntityID: ECS.EntityId, newSourceEntityID?: ECS.EntityId, newTargetEntityID?: ECS.EntityId, newInfo?: ECS.AttributeValue): void {
  const relationDetails = getAttribute(relationEntityID, 'relationDetails') as ECS.RelationDetail | null;
...
function removeAttribute(entityID: ECS.EntityId, attributeType: string, index = 0): void {
...
function removeAttributeByCriteria(entityID: ECS.EntityId, attributeType: string, criteria: ECS.AttributeValue): void {
  const index = getAttributeIndexByCriteria(entityID, attributeType, criteria);
  if (index !== -1) {
...
function getAttributeIndexByCriteria(entityID: ECS.EntityId, attributeType: string, criteria: ECS.AttributeValue): number {
...
function getFirstAttributeByType(attributeType: string): ECS.AttributeValue | null {
...
function getAttributes(entityID: ECS.EntityId, attributeType: string): ECS.AttributeValue[] {
  return attributeStore[attributeType]?.[entityID] || [];
}

function getAttribute(entityID: ECS.EntityId, attributeType: string, index = 0): ECS.AttributeValue | null {
  const attributes = getAttributes(entityID, attributeType);
...
function getRelation(relationEntityID: ECS.EntityId): ECS.RelationDetail | null {
  const relationAttributes = getAttributes(relationEntityID, 'relationDetails');
  return relationAttributes.length > 0 ? (relationAttributes[0] as ECS.RelationDetail) : null;
}


function queryEntitiesByAttribute(attributeType: string, criteria?: ECS.AttributeValue): ECS.EntityId[] {
  const typeMap = attributeStore[attributeType];
...
function queryEntitiesInRelationTo(targetEntityID: ECS.EntityId): ECS.EntityId[] {
  const getRelatedEntities = (index: { [entityID: string]: ECS.EntityId[] }, inverse: boolean) => {
...
function queryEntitiesByRelationTo(relationType: string, entityID: ECS.EntityId, isSource?: boolean): (ECS.EntityId | undefined)[] {
...
function destroyEntity(entityID: ECS.EntityId): void {
  ...
  for (const direction in Object.keys(relationIndex)) {
    for (const indexKey in directions) {
      const index = relationIndex[direction][indexKey as keyof typeof relationIndex[string]];
      ...
      if (index[entityID]) {
        for (const relationEntityID of index[entityID]) {
          ...
          if (relationDetails) {
            ...
            removeFromIndex(direction, oppositeEntityID, entityID, relationEntityID);
