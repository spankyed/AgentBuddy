...
var session = (0, import_sqlite_core.sqliteTable)("session", {
  id: (0, import_sqlite_core.text)("id").primaryKey(),
  model: (0, import_sqlite_core.text)("model").notNull(),
  createdAt: (0, import_sqlite_core.integer)("created_at", { mode: "timestamp" }).notNull()
});
var message = (0, import_sqlite_core.sqliteTable)("message", {
  id: (0, import_sqlite_core.text)("id").primaryKey(),
  sessionId: (0, import_sqlite_core.text)("session_id").notNull().references(() => session.id),
  role: (0, import_sqlite_core.text)("role", { enum: ["user", "assistant", "system"] }).notNull(),
...
var chatRouter = router({
  openSession: procedure.input(import_zod.z.object({ model: import_zod.z.string().default("gpt-4o") })).output(import_zod.z.object({ sessionId: import_zod.z.string() })).mutation(async ({ ctx, input }) => {
  ...
  userMessage: procedure.input(import_zod.z.object({ sessionId: import_zod.z.string(), content: import_zod.z.string().min(1) })).mutation(async ({ ctx, input }) => {
  ...
  onToken: procedure.input(import_zod.z.object({ sessionId: import_zod.z.string() })).subscription(
  ...
  abort: procedure.input(import_zod.z.object({ sessionId: import_zod.z.string() })).mutation(({ ctx, input }) => {
...
var sqlite = new import_better_sqlite32.default(process.env.DB_URL ?? "./dev.db");
...
// src/agents/agentMachine.ts
var agentMachine = (0, import_xstate.setup)({
  types: {
    context: {},
    events: {}
  },
  actions: {
    storePrompt: (0, import_xstate.assign)({
      userPrompt: ({ event }) => event.type === "USER_MSG" ? event.content : void 0
    }),
    spawnLlmTask: (0, import_xstate.assign)(({ context }) => {
...
async function runLlm(ctx, signal) {
  ...
  for await (const token of runner.stream(userPrompt, { signal })) {
    (0, import_xstate.sendParent)({ type: "TOKEN", token, sessionId });
  }
  (0, import_xstate.sendParent)({ type: "LLM_DONE" });
}
  ...
  return {
    ...
    getAgent(sessionId, model) {
      if (!actors.has(sessionId)) {
        const actor = (0, import_xstate2.createActor)(agentMachine, {
...
// src/server.ts
var wss = new import_ws2.WebSocketServer({ port: 3001, path: "/trpc" });
